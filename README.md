...
nothing. That's okay! You're going to write some code in there soon.
...
To run the tests, type `npm install` and then `npm test` in the terminal. You should now
...
- In the second `it` function call for `companyName`, it says it
  `is defined as a const`. The next line of code tests this.
